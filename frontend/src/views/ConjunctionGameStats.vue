<template>
    <div class="stats-page">
        ...
        <div class="page-header">
            <router-link to="/conjunction-game" class="back-link">← Back to game</router-link>
            <h1>🔗 Conjunction Game — Stats</h1>
        ...
        <template v-else>
            ...
            <!-- ══════════════════════════════════════════════
                 HARDEST CONJUNCTIONS LEADERBOARD
            ══════════════════════════════════════════════ -->
            <section class="section" v-if="stats.hardest_conjunctions?.length">
                <h2 class="section-title">⚠️ Hardest conjunctions</h2>
                <div class="mistakes-table">
                    ...
                    <div v-for="hc in mistakeLeaderboard" :key="hc.conjunction" class="mt-row">
                        <span class="mt-word">
                            {{ hc.conjunction }}
                            <WordBankButton :word="hc.conjunction" word-type="conjunction" category="Conjunction" />
                        </span>
                        <span class="mt-type">
                            <span class="type-pill" :class="'type-' + hc.conjunction_type">
                                {{ formatType(hc.conjunction_type) }}
                            </span>
                        </span>
                        <span class="mt-count">{{ hc.times_wrong }} / {{ hc.times_seen }}</span>
                        <span class="mt-bar-cell">
                            <div class="mt-bar-wrap">
                                <div class="mt-bar" :class="errorRateClass(hc.error_rate)"
                                    :style="{ width: hc.error_rate + '%' }"></div>
                            </div>
                            <span class="mt-pct">{{ hc.error_rate }}%</span>
                        </span>
                    </div>
                </div>
            </section>
                    ...
                    <div v-for="game in history" :key="game.id" class="history-card">
                        ...
                        <div v-if="expandedGames.has(game.id)" class="answer-list">
                            <div v-for="ans in game.answers" :key="ans.id" class="answer-row"
                                ...
                                <div class="ans-verb">
                                    <span class="ans-infinitive">{{ ans.conjunction }}</span>
                                    <span class="ans-meta" v-if="ans.conjunction_type">
                                        {{ formatType(ans.conjunction_type) }}
                                    </span>
                                </div>
                                ...
                                <div class="wb-quick-row">
                                    ...
                                    <WordBankButton :word="ans.conjunction" word-type="conjunction"
                                        category="Conjunction" :context-sentence="ans.sentence" />
            ...
            <div v-if="!history.length" class="empty-state">
                ...
                <router-link to="/conjunction-game" class="play-btn">Play your first game</router-link>
            </div>
        </template>
    </div>
</template>
...
async function fetchAll() {
    ...
    try {
        ...
        const [statsRes, historyRes] = await Promise.all([
            ...
            ax.get('/api/conjunction-game/history'),
        ])
...
const mistakeLeaderboard = computed(() =>
    (stats.value?.hardest_conjunctions ?? []).map(hc => ({
        ...hc,
        error_rate: hc.times_seen > 0
            ? Math.round((hc.times_wrong / hc.times_seen) * 100)
            : 0,
    }))
)
...
.mt-header {
    display: grid;
    grid-template-columns: 1.6fr 1.2fr 1.2fr 1.8fr;
    background: #f7fafc;
    padding: 10px 16px;
    font-weight: 700;
    color: #4a5568;
