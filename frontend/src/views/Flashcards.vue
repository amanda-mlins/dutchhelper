            ...
            <template v-else>
                ...
                <div class="mode-tabs">
                    ...
                    <button :class="['mode-tab', { active: setupMode === 'prep-verbs' }]"
                        ...
                        🎯 My Prep Pairs <span class="count-badge">{{ prepPairs.length }}</span>
                    </button>
                    <button :class="['mode-tab', { active: setupMode === 'discover' }]" @click="setupMode = 'discover'">
                        🔍 Discover <span class="count-badge">{{ allPrepPairsRaw.length }}</span>
                    </button>
                </div>
                    ...
                    <template v-else>
                        <div class="discover-header">
                            <p class="discover-desc">
                                Test yourself on <strong>{{ allPrepPairsRaw.length }}</strong> Dutch verb+preposition
                                combinations — no game history needed. Front shows the pair; back reveals the English
                                translation.
                            </p>
                        </div>
                        <div class="discover-count-row">
                            <span class="discover-count-label">How many cards?</span>
                            <div class="discover-count-options">
                                <button v-for="n in discoverCountOptions" :key="n"
                                    :class="['count-btn', { active: discoverCount === n }]" @click="discoverCount = n">
                                    {{ n === allPrepPairsRaw.length ? 'All' : n }}
                                </button>
                            </div>
                        </div>
                    </template>
                ...
                <button class="btn-start"
                    :disabled="(setupMode === 'pick' && pickedIds.size === 0) || (setupMode === 'category' && !selectedCategory) || (setupMode === 'prep-verbs' && prepPairs.length === 0) || (setupMode === 'discover' && allPrepPairsRaw.length === 0)"
                    @click="startSession">
...
export default {
    ...
    data() {
        return {
            ...
            // ----- prep-verb mode -----
            prepPairs: [],           // only LLM-generated (prep_sentence exists)
            allPrepPairsRaw: [],     // full catalogue (stubs + generated)
            isPrepLoading: false,
            prepLoadError: null,
            discoverCount: 20,
    ...
    computed: {
        ...
        discoverCountOptions() {
            const total = this.allPrepPairsRaw.length;
            return [10, 20, 30, total].filter((n, i, arr) => n <= total && arr.indexOf(n) === i);
        },
        ...
        deckSize() {
            ...
            if (this.setupMode === 'discover') return Math.min(this.discoverCount, this.allPrepPairsRaw.length);
    ...
    methods: {
        ...
        async fetchPrepPairs() {
            ...
            try {
                ...
                // Full catalogue for discover mode
                this.allPrepPairsRaw = data;
                // Only pairs with LLM-generated sentences for "My Prep Pairs" tab
        ...
        buildDeck() {
            ...
            if (this.setupMode === 'all') {
            ...
            } else if (this.setupMode === 'discover') {
                // Full catalogue, randomly sampled to discoverCount
                const pool = [...this.allPrepPairsRaw];
                // Shuffle the pool
                ...
                // discover mode is always shuffled — skip the second shuffle below
                return source;
            } else {
...
.discover-count-label {
    font-weight: 600;
    font-size: 14px;
    color: #444;
}

.discover-count-options {
    display: flex;
    gap: 8px;
    flex-wrap: wrap;
}
