<template>
    <div class="stats-page">
        ...
        <div class="page-header">
            <router-link to="/article-game" class="back-link">← Back to game</router-link>
            <h1>🎮 Article Game — Stats</h1>
        ...
        <template v-else>
            ...
            <section class="summary-section">
                <div class="summary-grid">
                    ...
                    <div class="summary-card accent-blue">
                        <div class="sc-value">{{ stats.words_studied }}</div>
                        <div class="sc-label">Unique words seen</div>
                    </div>
            ...
            ══════════════════════════════════════════════ -->
            <section class="section" v-if="stats.hardest_words?.length">
                <h2 class="section-title">⚠️ Hardest words</h2>
                <div class="mistakes-table">
                    <div class="mt-header">
                        <span>Word</span>
                        <span>Correct article</span>
                        <span>Wrong / Seen</span>
                        <span>Error rate</span>
                    </div>
                    <div v-for="hw in mistakeLeaderboard" :key="hw.word" class="mt-row">
                        <span class="mt-word">{{ hw.word }}</span>
                        <span class="mt-article">{{ hw.correct_article }}</span>
                        <span class="mt-count">{{ hw.times_wrong }} / {{ hw.times_seen }}</span>
                        <span class="mt-bar-cell">
                            <div class="mt-bar-wrap">
                                <div class="mt-bar" :class="errorRateClass(hw.error_rate)"
                                    :style="{ width: hw.error_rate + '%' }"></div>
                            </div>
                            <span class="mt-pct">{{ hw.error_rate }}%</span>
                        </span>
                    </div>
                </div>
            </section>
                    ...
                    <div v-for="game in history" :key="game.id" class="history-card">
                        ...
                        <button class="history-header" @click="toggleGame(game.id)">
                            <div class="hh-left">
                                ...
                                <span class="hh-size">{{ game.word_count }} words</span>
                            </div>
                            <div class="hh-right">
                                <span class="hh-score">{{ game.score }} / {{ game.word_count }}</span>
                        ...
                        <div v-if="expandedGames.has(game.id)" class="answer-grid">
                            <div v-for="ans in game.answers" :key="ans.word" class="answer-chip"
                                :class="ans.is_correct ? 'chip-correct' : 'chip-wrong'">
                                <span class="chip-article">{{ ans.user_answer }}</span>
                                <span class="chip-word">{{ ans.word }}</span>
                                <span v-if="!ans.is_correct" class="chip-correct-hint">
                                    ({{ ans.correct_article }})
                                </span>
                            </div>
                        </div>
            ...
            <div v-if="!history.length" class="empty-state">
                ...
                <router-link to="/article-game" class="play-btn">Play your first game</router-link>
            </div>
        </template>
    </div>
</template>
...
async function fetchAll() {
    ...
    try {
        ...
        const [statsRes, historyRes] = await Promise.all([
            authAxios.get('/api/game/stats'),
            authAxios.get('/api/game/history'),
        ])
...
/** Hardest words enriched with a computed error_rate field. */
const mistakeLeaderboard = computed(() =>
    (stats.value?.hardest_words ?? []).map(hw => ({
        ...hw,
        error_rate: hw.times_seen > 0
            ? Math.round((hw.times_wrong / hw.times_seen) * 100)
            : 0,
    }))
)
